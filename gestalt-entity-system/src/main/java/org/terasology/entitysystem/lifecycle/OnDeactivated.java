 ...
 *
 */
public class OnDeactivated extends LifecycleEvent {
    private TypeKeyedMap<Component> components;

    public OnDeactivated(int revision, TypeKeyedMap<Component> components) {
        super(revision);
        this.components = new TypeKeyedMap<>(Collections.unmodifiableMap(components.getInner()));
    }

    public OnDeactivated(int revision, Collection<Component> components) {
        super(revision);
